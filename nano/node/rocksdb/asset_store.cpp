...
nano::rocksdb::asset_store::asset_store (nano::rocksdb::store & store) :
	store{ store } {};

void nano::rocksdb::asset_store::put (nano::write_transaction const & transaction, nano::uint256_union const & asset_id, nano::asset_info const & info)
{
	auto status = store.put (transaction, tables::assets, asset_id, info);
	store.release_assert_success (status);
}

bool nano::rocksdb::asset_store::get (nano::transaction const & transaction, nano::uint256_union const & asset_id, nano::asset_info & info_a)
{
...
void nano::rocksdb::asset_store::del (nano::write_transaction const & transaction, nano::uint256_union const & asset_id)
{
	auto status = store.del (transaction, tables::assets, asset_id);
	store.release_assert_success (status);
}

bool nano::rocksdb::asset_store::exists (nano::transaction const & transaction, nano::uint256_union const & asset_id)
{
...
size_t nano::rocksdb::asset_store::count (nano::transaction const & transaction)
{
	return store.count (transaction, tables::assets);
}

nano::store_iterator<nano::uint256_union, nano::asset_info> nano::rocksdb::asset_store::begin (nano::transaction const & transaction) const
{
	return store.template make_iterator<nano::uint256_union, nano::asset_info> (transaction, tables::assets);
}

nano::store_iterator<nano::uint256_union, nano::asset_info> nano::rocksdb::asset_store::begin (nano::transaction const & transaction, nano::uint256_union const & asset_id) const
{
	return store.template make_iterator<nano::uint256_union, nano::asset_info> (transaction, tables::assets, asset_id);
}

nano::store_iterator<nano::uint256_union, nano::asset_info> nano::rocksdb::asset_store::end () const
{
	return nano::store_iterator<nano::uint256_union, nano::asset_info> (nullptr);
}

void nano::rocksdb::asset_store::balance_put (nano::write_transaction const & transaction, nano::account const & account, nano::uint256_union const & asset_id, nano::amount const & balance)
{
...
void nano::rocksdb::asset_store::balance_del (nano::write_transaction const & transaction, nano::account const & account, nano::uint256_union const & asset_id)
{
...
nano::amount nano::rocksdb::asset_store::balance (nano::transaction const & transaction, nano::account const & account, nano::uint256_union const & asset_id)
{
...
size_t nano::rocksdb::asset_store::holdings_count (nano::transaction const & transaction, nano::account const & account)
{
	size_t result (0);
...
nano::store_iterator<nano::asset_key, nano::amount> nano::rocksdb::asset_store::holdings_begin (nano::transaction const & transaction, nano::asset_key const & key) const
{
	return store.template make_iterator<nano::asset_key, nano::amount> (transaction, tables::holdings, key);
}

nano::store_iterator<nano::asset_key, nano::amount> nano::rocksdb::asset_store::holdings_begin (nano::transaction const & transaction) const
{
	return store.template make_iterator<nano::asset_key, nano::amount> (transaction, tables::holdings);
}

nano::store_iterator<nano::asset_key, nano::amount> nano::rocksdb::asset_store::holdings_end () const
{
	return nano::store_iterator<nano::asset_key, nano::amount> (nullptr);
}

nano::store_iterator<nano::asset_key, nano::amount> nano::rocksdb::asset_store::holders_begin (nano::transaction const & transaction, nano::asset_key const & key) const
{
	return store.template make_iterator<nano::asset_key, nano::amount> (transaction, tables::holders, key);
}

nano::store_iterator<nano::asset_key, nano::amount> nano::rocksdb::asset_store::holders_begin (nano::transaction const & transaction) const
{
	return store.template make_iterator<nano::asset_key, nano::amount> (transaction, tables::holders);
}

nano::store_iterator<nano::asset_key, nano::amount> nano::rocksdb::asset_store::holders_end () const
{
	return nano::store_iterator<nano::asset_key, nano::amount> (nullptr);
}
...
bool nano::rocksdb::asset_store::pending_get (nano::transaction const & transaction, nano::pending_key const & key, nano::asset_pending_info & info_a)
{
	nano::rocksdb_val value;
...
void nano::rocksdb::asset_store::pending_del (nano::write_transaction const & transaction, nano::pending_key const & key)
{
	auto status = store.del (transaction, tables::asset_pending, key);
	store.release_assert_success (status);
}

bool nano::rocksdb::asset_store::pending_exists (nano::transaction const & transaction, nano::pending_key const & key)
{
	auto iterator (pending_begin (transaction, key));
	return iterator != pending_end () && nano::pending_key (iterator->first) == key;
}

nano::store_iterator<nano::pending_key, nano::asset_pending_info> nano::rocksdb::asset_store::pending_begin (nano::transaction const & transaction, nano::pending_key const & key) const
{
	return store.template make_iterator<nano::pending_key, nano::asset_pending_info> (transaction, tables::asset_pending, key);
}

nano::store_iterator<nano::pending_key, nano::asset_pending_info> nano::rocksdb::asset_store::pending_begin (nano::transaction const & transaction) const
{
	return store.template make_iterator<nano::pending_key, nano::asset_pending_info> (transaction, tables::asset_pending);
}

nano::store_iterator<nano::pending_key, nano::asset_pending_info> nano::rocksdb::asset_store::pending_end () const
{
...
void nano::rocksdb::asset_store::lock_put (nano::write_transaction const & transaction, nano::block_hash const & offer, nano::asset_lock_info const & info)
{
	auto status = store.put (transaction, tables::swap_locks, offer, info);
	store.release_assert_success (status);
}

bool nano::rocksdb::asset_store::lock_get (nano::transaction const & transaction, nano::block_hash const & offer, nano::asset_lock_info & info_a)
{
	nano::rocksdb_val value;
...
void nano::rocksdb::asset_store::lock_del (nano::write_transaction const & transaction, nano::block_hash const & offer)
{
	auto status = store.del (transaction, tables::swap_locks, offer);
	store.release_assert_success (status);
}

bool nano::rocksdb::asset_store::lock_exists (nano::transaction const & transaction, nano::block_hash const & offer)
{
	nano::rocksdb_val junk;
...
nano::store_iterator<nano::block_hash, nano::asset_lock_info> nano::rocksdb::asset_store::locks_begin (nano::transaction const & transaction, nano::block_hash const & offer) const
{
	return store.template make_iterator<nano::block_hash, nano::asset_lock_info> (transaction, tables::swap_locks, offer);
}

nano::store_iterator<nano::block_hash, nano::asset_lock_info> nano::rocksdb::asset_store::locks_begin (nano::transaction const & transaction) const
{
	return store.template make_iterator<nano::block_hash, nano::asset_lock_info> (transaction, tables::swap_locks);
}

nano::store_iterator<nano::block_hash, nano::asset_lock_info> nano::rocksdb::asset_store::locks_end () const
{
	return nano::store_iterator<nano::block_hash, nano::asset_lock_info> (nullptr);
}

void nano::rocksdb::asset_store::offer_put (nano::write_transaction const & transaction, nano::uint256_union const & asset_id, nano::block_hash const & offer, nano::account const & offerer)
{
	auto status = store.put (transaction, tables::swap_offers, nano::offer_key (asset_id, offer), offerer);
	store.release_assert_success (status);
}

void nano::rocksdb::asset_store::offer_del (nano::write_transaction const & transaction, nano::uint256_union const & asset_id, nano::block_hash const & offer)
{
	auto status = store.del (transaction, tables::swap_offers, nano::offer_key (asset_id, offer));
	store.release_assert_success (status);
}

nano::store_iterator<nano::asset_key, nano::account> nano::rocksdb::asset_store::offers_begin (nano::transaction const & transaction, nano::asset_key const & key) const
{
	return store.template make_iterator<nano::asset_key, nano::account> (transaction, tables::swap_offers, key);
}

nano::store_iterator<nano::asset_key, nano::account> nano::rocksdb::asset_store::offers_begin (nano::transaction const & transaction) const
{
	return store.template make_iterator<nano::asset_key, nano::account> (transaction, tables::swap_offers);
}

nano::store_iterator<nano::asset_key, nano::account> nano::rocksdb::asset_store::offers_end () const
{
